...
pub struct MySVGWriter<Vertex:Display> {
    content: String,
    view_size: Point,
    counter: usize,
...
impl <T:Display> Drop for MySVGWriter<T> {
    fn drop(&mut self) {}
}

impl <T:Display>MySVGWriter<T> {
    ...
    pub fn finalize(&self) -> String {
        ...
        for (idx,id) in self.vertex_ids.iter().enumerate(){
            content = content.replace(&format!("::{idx}::"),id );
        }
...
impl <T:Display> RenderBackend for MySVGWriter<T> {
    fn draw_rect(
        &mut self,
        xy: Point,
        size: Point,
    ...
    fn draw_arrow(
        ...
        //dbg!("draw_arrow",&path,&self.points);
        ...
        let line = format!(
            "<text><textPath href=\"#arrow{}\" startOffset=\"50%\" \
             text-anchor=\"middle\" class=\"{}\">{}</textPath></text></g>",
            self.counter,
            font_class,
            escape_string(text)
        );
